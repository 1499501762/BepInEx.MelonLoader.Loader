...
namespace MelonLoader
{
    internal static class Core
    {
        ...
        internal static int Initialize()
        {
            ...
            if (BootstrapInterop.Library != null)
                BootstrapInterop.Library.GetLoaderConfig(ref config);
            else
                config = LoaderConfig.Current; // BepInEx-hosted mode (BepInExHost sets LoaderConfig.Current)
            ...
            try
            {
                // Under BepInEx hosting the mono runtime is already set up by BepInEx,
                // so we don't need to resolve the native mono library ourselves.
                if (!Hosting.BepInExHost.IsActive && !MonoLibrary.Setup())
                {
                    _success = false;
                    return 1;
                }
            ...
            // Under BepInEx hosting the plugin entry point (Awake/Load) already runs at the right time.
        ...
        internal static bool Start()
        {
            ...
            // Under BepInEx hosting the start screen is not shown and the pre-setup
            // (Il2CppAssemblyGenerator) is invoked directly.
            if (Hosting.BepInExHost.IsActive)
                PreSetup();
            else
